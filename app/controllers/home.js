      ...
      db.Event.create({ name: body.name, category: body.category, email: body.email, phone: body.phone, when: body.time, publish: body.publish }).then(function (response) {
        ...
        if(body.recibir){
          ...
          var address = {
              ...
              html: "Tu cita "  + cita + " para el " +moment.tz(body.time, "GMT+0").format("L")+" "+moment.tz(body.time, "GMT+0").format("LTS")+  " ha sido agendada otra hora, nuestro personal se comunicará contigo en las próximas horas."
          };
